import { Adapter, WebSocketReceiver, WSSReceiver, WebhookReceiver, SSEReceiver, Message } from '@imhelper/client';
...
export function createOnebot12Adapter(config: OneBotV12AdapterConfig): Adapter<string, string | OneBotV12Segment[], OneBotV12Response> {
  ...
  class OneBotV12AdapterImpl extends Adapter<string, string | OneBotV12Segment[], OneBotV12Response> {
    ...
    constructor() {
      ...
      this.setupReceiver();
      // 监听 receiver 触发的事件
      (this as any).on('event', (event: OneBotV12Event) => this.transformAndEmit(event));
    }
