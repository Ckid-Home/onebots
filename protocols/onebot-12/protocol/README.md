# @imhelper/protocol-onebot-12

imhelper OneBot V12 协议实现 - 支持 OneBot 12 标准的协议插件

## 简介

`@imhelper/protocol-onebot-12` 是 imhelper 框架的官方 OneBot V12 协议实现，完全兼容 [OneBot 12 标准](https://12.onebot.dev)，提供更现代化和标准化的机器人通信协议。
...
# 或
pnpm add @imhelper/protocol-onebot-12
```
...
使用 `imhelper` 命令行工具时，通过 `-p` 参数注册协议：

```bash
# 注册 OneBot V12 协议
imhelper -p onebot-v12

# 同时注册多个协议
imhelper -p onebot-v11 -p onebot-v12

# 注册协议并指定适配器和配置
imhelper -r wechat -p onebot-v12 -c config.yaml
```

协议会自动从以下位置加载：
- `@imhelper/protocol-onebot-12` (官方包)
- `imhelper-protocol-onebot-v12` (社区包)
- `onebot-v12` (直接包名)
...
import { OneBotV12Protocol } from '@imhelper/protocol-onebot-12';
...
imhelper 主动推送事件到配置的 HTTP Webhook 地址。

### WebSocket Reverse

imhelper 主动连接到配置的 WebSocket 地址。
