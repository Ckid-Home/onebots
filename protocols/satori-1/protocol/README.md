# @imhelper/protocol-satori-1-v1

imhelper Satori V1 协议实现 - 支持 Satori 协议的插件

## 简介

`@imhelper/protocol-satori-1-v1` 是 imhelper 框架的官方 Satori 协议实现。Satori 是一个现代化的跨平台机器人协议，专注于提供统一的消息格式和事件系统。
...
# 或
pnpm add @imhelper/protocol-satori-1-v1
```
...
```bash
# 注册 Satori V1 协议
imhelper -p satori-v1

# 同时注册多个协议
imhelper -p satori-v1 -p onebot-v11 -p onebot-v12

# 注册协议并指定适配器
imhelper -r wechat -p satori-v1 -c config.yaml
```

协议会自动从以下位置加载：
- `@imhelper/protocol-satori-1-v1` (官方包)
- `imhelper-protocol-satori-v1` (社区包)
- `satori-v1` (直接包名)
...
import { SatoriV1 } from '@imhelper/protocol-satori-1-v1';
