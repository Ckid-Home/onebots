# @imhelper/protocol-milky-v1

imhelper Milky V1 协议实现 - 支持 Milky 协议的插件

## 简介

`@imhelper/protocol-milky-v1` 是 imhelper 框架的官方 Milky V1 协议实现。Milky 是一个类似 OneBot 的 QQ 机器人协议，提供了不同的消息格式和 API 设计。
...
# 或
pnpm add @imhelper/protocol-milky-v1
```
...
# 同时注册多个协议
imhelper -p milky-v1 -p onebot-v11 -p satori-v1

# 注册协议并指定适配器
imhelper -r qq -p milky-v1 -c config.yaml
```

协议会自动从以下位置加载：
- `@imhelper/protocol-milky-v1` (官方包)
- `imhelper-protocol-milky-v1` (社区包)
- `milky-v1` (直接包名)
...
import { MilkyV1 } from '@imhelper/protocol-milky-v1';
...
imhelper 主动推送事件到配置的 HTTP 地址，支持 HMAC 签名验证。
