...
多平台多协议的机器人应用启动器 - imhelper 主应用包

## 简介

`imhelper` 是 imhelper 框架的主应用包，提供了完整的应用层功能，包括配置管理、适配器加载、协议注册等。它基于 `@imhelper/core` 核心库，为开发者提供开箱即用的机器人应用解决方案。
...
# 指定配置文件
npx imhelper -c config.yaml

# 注册适配器和协议
npx imhelper -r wechat -p onebot-v11 -p satori-v1
```
...
imhelper 使用 YAML 格式的配置文件，采用**账号标识 + 协议配置**的方式：
...
**协议:**
- `@imhelper/protocol-{name}-{version}`
- `imhelper-protocol-{name}-{version}`
- `{name}`
...
- [@imhelper/adapter-wechat](../adapter-wechat) - 微信公众号适配器
...
- [imhelper 文档](../../docs)
- [GitHub 仓库](https://github.com/lc-cn/imhelper)
