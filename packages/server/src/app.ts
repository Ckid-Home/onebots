import { BaseApp, yaml, AdapterRegistry,RouterContext,Next, ProtocolRegistry, configure, Protocol, Adapter, readLine } from "@imhelper/core";
import * as path from "path";
import * as fs from "fs";
import { createRequire } from "module";
import koaStatic from "koa-static";
import { copyFileSync, existsSync, writeFileSync, mkdirSync, readFileSync } from "fs";
import type { WsServer, Dict } from "@imhelper/core";
import * as pty from "@karinjs/node-pty";

const require = createRequire(import.meta.url);
const client = path.resolve(import.meta.dirname, '../node_modules/@imhelper/web/dist');
...
export namespace App {
    ...
    export async function loadProtocolFactory(name: string, maybeNames=[
        ...
        try{
            ...
            console.warn(`[imhelper] Failed to load protocol ${modName}: ${e}`);
    ...
    configure({
        appenders: {
            ...
            files: {
                ...
                filename: path.join(process.cwd(), "imhelper.log"),
            },
        },
        categories: {
            default: {
